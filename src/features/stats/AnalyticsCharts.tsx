...
export function AnalyticsCharts({
  ...
  // Cálculo de Métricas Financieras Reales basadas en los datos de rentals
  const metrics = React.useMemo(() => {
    let paidTotal = 0;
    let pendingTotal = 0;
    let totalOps = rentals.length;
    ...
    const netProfit = paidTotal - Math.round(pendingTotal * 0.15); // Estimación neta
    return {
  ...
  // Datos simulados/dinámicos para el Gráfico de Barras (Rendimiento)
  const barData = [
    { label: '01', value: 30 },
    { label: '02', value: 52 },
    { label: '03', value: 35 },
  ...
  // Puntos para la curva suavizada (Dual Line Chart)
  ...
  return (
    <View style={styles.dashboardContainer}>
      {/* 1. Header Toolbar Superior de Control */}
      <View style={styles.topToolbar}>
        {/* Pills de navegación: Overview / Ventas / Gastos */}
        <View style={[styles.navTabsPill, { backgroundColor: isDark ? '#161B22' : '#EFF2F5', borderColor: theme.surfaceBorder }]}>
          <Pressable
            style={[styles.navTabItem, activeTab === 'overview' && styles.navTabItemActive]}
          ...
          <Pressable
            style={[styles.navTabItem, activeTab === 'sales' && styles.navTabItemActive]}
            onPress={() => setActiveTab('sales')}
          >
            <Text style={[styles.navTabText, activeTab === 'sales' && styles.navTabTextActive]}>
              Ventas
            </Text>
          </Pressable>
          <Pressable
            style={[styles.navTabItem, activeTab === 'expenses' && styles.navTabItemActive]}
            onPress={() => setActiveTab('expenses')}
          >
            <Text style={[styles.navTabText, activeTab === 'expenses' && styles.navTabTextActive]}>
              Gastos
            </Text>
          </Pressable>
        </View>

        {/* Acciones de la derecha: Refresco, Selector de Periodo y Download */}
        <View style={styles.rightActionsGroup}>
          ...
          <View style={[styles.periodSelectorPill, { backgroundColor: isDark ? '#1C2128' : '#EFF2F5', borderColor: theme.surfaceBorder }]}>
      ...
      {/* 2. Grid de 4 Tarjetas de Indicadores Principales (KPIs) */}
      <View style={styles.kpiGrid}>
        {/* Card 1: Revenue (Ingresos) */}
        <View style={[styles.kpiCard, { backgroundColor: isDark ? '#161B22' : '#FFFFFF', borderColor: theme.surfaceBorder }]}>
          <Text style={[styles.kpiLabel, { color: theme.textMuted }]}>Revenue</Text>
          <View style={styles.kpiValueRow}>
            <Text style={[styles.kpiValue, { color: theme.text }]}>
              ${metrics.revenue.toLocaleString('es-AR')} US$
            </Text>
        ...
        {/* Card 2: Expenses (Gastos / Pendientes) */}
        <View style={[styles.kpiCard, { backgroundColor: isDark ? '#161B22' : '#FFFFFF', borderColor: theme.surfaceBorder }]}>
          <Text style={[styles.kpiLabel, { color: theme.textMuted }]}>Expenses</Text>
          <View style={styles.kpiValueRow}>
            <Text style={[styles.kpiValue, { color: theme.text }]}>
              ${metrics.expenses.toLocaleString('es-AR')} US$
            </Text>
            <View style={styles.badgeDanger}>
              <Feather name="arrow-down" size={11} color="#EF4444" />
              <Text style={styles.badgeDangerText}>3.3%</Text>
            </View>
          </View>
        </View>

        {/* Card 3: Sales (Operaciones Totales) */}
        <View style={[styles.kpiCard, { backgroundColor: isDark ? '#161B22' : '#FFFFFF', borderColor: theme.surfaceBorder }]}>
          <Text style={[styles.kpiLabel, { color: theme.textMuted }]}>Sales</Text>
          <View style={styles.kpiValueRow}>
            <Text style={[styles.kpiValue, { color: theme.text }]}>
              {metrics.salesCount}
            </Text>
        ...
        {/* Card 4: Profit (Ganancia Neta) */}
        <View style={[styles.kpiCard, { backgroundColor: isDark ? '#161B22' : '#FFFFFF', borderColor: theme.surfaceBorder }]}>
          <Text style={[styles.kpiLabel, { color: theme.textMuted }]}>Profit</Text>
          <View style={styles.kpiValueRow}>
            <Text style={[styles.kpiValue, { color: theme.text }]}>
              ${metrics.profit.toLocaleString('es-AR')} US$
            </Text>
            <View style={styles.badgeSuccess}>
              <Feather name="arrow-up" size={11} color="#22C55E" />
              <Text style={styles.badgeSuccessText}>4.1%</Text>
            </View>
          </View>
        </View>
      </View>

      {/* 3. Fila Doble de Gráficos de Analítica */}
      <View style={styles.chartsGridRow}>
        {/* GRÁFICO 1: Sales Performance (Bar Chart) */}
        <View style={[styles.chartCard, { backgroundColor: isDark ? '#161B22' : '#FFFFFF', borderColor: theme.surfaceBorder }]}>
          <View style={styles.chartHeader}>
            <Text style={[styles.chartTitle, { color: theme.text }]}>Sales Performance</Text>
            <View style={[styles.filterDropdownPill, { backgroundColor: isDark ? '#21262D' : '#F1F5F9' }]}>
          ...
          {/* Renderizado de Gráfico de Barras Nativas */}
          <View style={styles.barChartContainer}>
            ...
            <View style={styles.barsPlotArea}>
              ...
              <View style={styles.barsRow}>
                {barData.map((b, idx) => {
                  const heightPercent = `${(b.value / maxBarValue) * 100}%`;
                  return (
        ...
        {/* GRÁFICO 2: Traffic Source / Evolución Temporal (Dual Line Chart) */}
        <View style={[styles.chartCard, { backgroundColor: isDark ? '#161B22' : '#FFFFFF', borderColor: theme.surfaceBorder }]}>
          <View style={styles.chartHeader}>
          ...
          {/* Renderizado de Gráfico de Líneas Suaves */}
          <View style={styles.lineChartContainer}>
            ...
            <View style={styles.linePlotArea}>
              ...
              {/* Trazado de Puntos y Conectores de Líneas */}
              <View style={styles.lineSeriesWrapper}>
                {months.map((m, idx) => {
                  const valPaid = lineDataPaid[idx];
                  const valPending = lineDataPending[idx];
                  const bottomPaid = `${(valPaid / maxLineValue) * 100}%`;
                  const bottomPending = `${(valPending / maxLineValue) * 100}%`;

                  return (
                    <View key={idx} style={styles.lineColItem}>
                      <View style={styles.linePlotTrack}>
                        <View style={[styles.lineDot, { bottom: bottomPaid as any, backgroundColor: '#0084FF' }]} />
                        <View style={[styles.lineDot, { bottom: bottomPending as any, backgroundColor: '#38BDF8' }]} />
                      </View>
